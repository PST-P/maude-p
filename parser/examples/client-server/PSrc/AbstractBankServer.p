    ...
    on eWithDrawReq do (wReq: tWithDrawReq) {
      ...
      if(balance[wReq.accountId] - wReq.amount > 10) // hint: bug 
      {
        ...
        send wReq.source, eWithDrawResp, 
          (status = WITHDRAW-SUCCESS, accountId = wReq.accountId, balance = balance[wReq.accountId], rId = wReq.rId);
      }
      else
      {
        send wReq.source, eWithDrawResp, 
          (status = WITHDRAW-ERROR, accountId = wReq.accountId, balance = balance[wReq.accountId], rId = wReq.rId);
      }
    }
  }
}
